function [Ts, tfr0, t0, f, GroupDelay] = TET_2nd_hop_gpu(x, Hz, sigma, hop)
    ...
    % 添加时间偏移（向量化）
    del_t = hop / Hz;
    tau_vec = gpuArray(single(1:tLen));  % GPU上的向量
    ...
    % 创建b索引矩阵（必须在GPU上创建！）
    time_indices_gpu = gpuArray(single(1:tLen));  % 关键修复！
    b_mat_gpu = repmat(time_indices_gpu, fLen, 1);  % 现在完全在GPU上
    
    % 找到m==b的位置
    equal_mask = (round(GroupDelay_gpu) == b_mat_gpu);
    
    % 直接赋值
    Ts_gpu(equal_mask) = tfr0_gpu(equal_mask);
