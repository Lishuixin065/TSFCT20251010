...
xlabel('Group delay Dispersion','FontSize',20);
set(gca,'FontSize',20)
...
%% 2nd order time-reassigned operators
[tfc0,tfc1,tfc2,tfrtic,tcrtic,T,R] = SFCT2(y,Hz,sigma,arange);

%% squeezeing transform
 [tfrsq1] = Msqueeze0(y, tfc0, R, T,t,tcrtic, 1,0.001);
 
% ridge detection
  [GDs,GDDs]=ridge_3d(tfrsq1,2,10,10,10);
...
if sum(abs(y1-trecov(1,:)).^2) <sum(abs(y2-trecov(1,:)).^2)
 ...
 figure, plot(t, real(y2-trecovr(1,:)), 'color', 'k');
 xlim([t(1) t(end)]);
  ylim([-0.1 0.1])
  xlabel('Time (s)','FontSize',20);
  set(gca,'FontSize',20)
